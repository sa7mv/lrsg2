...
exports.run = async (client, msg, args) => {
  ...
  if (member.id === "520213732752621568")
    return msg.channel.send({
      embed: {
        color: Math.floor(Math.random() * (0xffffff + 1)),
        description:
  ...
  if (member.id === "0")
    return msg.channel.send({
      embed: {
        color: Math.floor(Math.random() * (0xffffff + 1)),
        description: `:no_entry_sign: Hoop! ne yaptın sen @MaLZVr#2472 sahibimin kankasını etiketlemişsin yarram!`
      }
    });
  if (member.id === "777597876989722664")
    return msg.channel.send({
      embed: {
        color: Math.floor(Math.random() * (0xffffff + 1)),
        description: `:no_entry_sign: Hoop! Orda durucan sahibimin kuzeni o @ROXY#4383'e sövemezsin amını sikerim.`
      }
    });
