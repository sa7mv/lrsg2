...
message.channel.send(embd.setDescription(arr[0]).setFooter(`Sayfa ${page+1} / ${arr.length}`)).then(async msg => {
      await msg.react("⬅️");
      await msg.react("➡️");
        ...
        switch (reaction.emoji.name) {
          ...
          case "➡️":
            ...
            reaction.users.remove(user).catch(console.error);
              embd.setColor("RANDOM");
              embd.setFooter(`Sayfa ${page+1} / ${arr.length}`);
              embd.setDescription(arr[page] || `MAL ${arr.length} SAYFA EMOJİN VAR ${page+1}.SAYFAYA NİYE GEÇİYON`)
            msg.edit(embd)
          break;
